...
# - If user did not provide -arch (or provided an empty value), default to host architecture.
# - Otherwise, respect the user-specified target architecture (normalized to lowercase).
if (-not $PSBoundParameters.ContainsKey('arch') -or [string]::IsNullOrWhiteSpace($arch)) {
  $arch = $vs_host_arch
}
else {
  $arch = $arch.ToLower()
...
function CloneGitRepo {
  ...
  try {
    ...
    Write-Warning "Failed to close git repository $git_repo_name : $_"
    throw
  }
}
...
function Build-Yasm {
  ...
  try {
    ...
    Set-Location "yasm"
    & patch -p1 -N -i "$downloads_path\yasm-cmake.patch"
    # 2>&1 | Out-Null
    CMakeBuild
  }
  finally {
...
function Build-LibIconv {
  ...
  Push-Location $build_path
  try {
    CloneGitRepo -git_repo_name "libiconv"
    if (-not (Test-Path "libiconv-for-Windows")) {
      Copy-Item "$downloads_path\libiconv-for-Windows" "." -Recurse -Force
    }
    Set-Location "libiconv-for-Windows"
    UpgradeVSProject "libiconv.sln"
    MSBuildProject -project_path "libiconv.sln" -configuration "$build_type"
    Copy-Item "output\${msbuild_platform}\$build_type\*.lib" "$prefix_path\lib\" -Force
    Copy-Item "output\${msbuild_platform}\$build_type\*.dll" "$prefix_path\bin\" -Force
...
function Build-Pixman {
  ...
  try {
    ...
    try {
      ExtractPackage "pixman-$pixman_version.tar.gz"
    }
    catch {} # Ignore errors
...
try {
  ...
  #if (-not (Test-Path "$prefix_path\lib\libiconv*.lib")) { $build_queue += "libiconv" }
  ...
  if (-not (Test-Path "$prefix_path\lib\pkgconfig\tiff.pc")) { $build_queue += "tiff" }
